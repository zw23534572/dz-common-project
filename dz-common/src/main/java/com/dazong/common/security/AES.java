...
public class AES {
    ...
    public static void trace_static() {
        ...
        for (i = 0; i < 16; i++) {
            for (j = 0; j < 16; j++) System.out.print(ByteHexUtil.toHEX1(S[i * 16 + j]) + ", ");
            System.out.println();
        }
        System.out.print("Si[] = n");
        for (i = 0; i < 16; i++) {
            for (j = 0; j < 16; j++) System.out.print(ByteHexUtil.toHEX1(Si[i * 16 + j]) + ", ");
            System.out.println();
        }
        System.out.print("rcon[] = n");
        for (i = 0; i < 5; i++) {
            for (j = 0; j < 6; j++) System.out.print(ByteHexUtil.toHEX1(rcon[i * 6 + j]) + ", ");
            System.out.println();
        }
        System.out.print("log[] = n");
        for (i = 0; i < 32; i++) {
            for (j = 0; j < 8; j++) System.out.print(ByteHexUtil.toHEX1(log[i * 8 + j]) + ", ");
            System.out.println();
        }
        System.out.print("alog[] = n");
        for (i = 0; i < 32; i++) {
            for (j = 0; j < 8; j++) System.out.print(ByteHexUtil.toHEX1(alog[i * 8 + j]) + ", ");
            System.out.println();
        }
    }
    ...
    public byte[] encrypt(byte[] plain) {
        ...
        if (traceLevel > 0) traceInfo = "encryptAES(" + ByteHexUtil.toHEX1(plain) + ")";
        ...
        if (traceLevel > 2)
            traceInfo += "n  R0 (Key = " + ByteHexUtil.toHEX1(Ker) + ")ntAK = " + ByteHexUtil.toHEX1(a);
        else if (traceLevel > 1)
            traceInfo += "n  R0 (Key = " + ByteHexUtil.toHEX1(Ker) + ")t = " + ByteHexUtil.toHEX1(a);

        // for each round except last, apply round transforms
        for (int r = 1; r < numRounds; r++) {
            Ker = Ke[r];            // get session keys for this round
            if (traceLevel > 1) traceInfo += "n  R" + r + " (Key = " + ByteHexUtil.toHEX1(Ker) + ")t";

            // SubBytes(state) into ta using S-Box S
            for (i = 0; i < BLOCK_SIZE; i++) ta[i] = S[a[i] & 0xFF];
            if (traceLevel > 2) traceInfo += "ntSB = " + ByteHexUtil.toHEX1(ta);
            ...
            if (traceLevel > 2) traceInfo += "ntSR = " + ByteHexUtil.toHEX1(a);
        ...
        for (i = 0; i < BLOCK_SIZE; i++) a[i] = (byte) (ta[i] ^ Ker[i]);
        if (traceLevel > 2) traceInfo += "ntAK";
        if (traceLevel > 1) traceInfo += " = " + ByteHexUtil.toHEX1(a) + "n";
        if (traceLevel > 0) traceInfo += " = " + ByteHexUtil.toHEX1(a) + "n";
        return (a);
    }
    ...
    public byte[] decrypt(byte[] cipher) {
        ...
        traceInfo = "";                // reset trace info
        if (traceLevel > 0) traceInfo = "decryptAES(" + ByteHexUtil.toHEX1(cipher) + ")";
        ...
        if (traceLevel > 2)
            traceInfo += "n  R0 (Key = " + ByteHexUtil.toHEX1(Kdr) + ")nt AK = " + ByteHexUtil.toHEX1(a);
        else if (traceLevel > 1)
            traceInfo += "n  R0 (Key = " + ByteHexUtil.toHEX1(Kdr) + ")t = " + ByteHexUtil.toHEX1(a);

        // for each round except last, apply round transforms
        for (int r = 1; r < numRounds; r++) {
            Kdr = Kd[r];            // get session keys for this round
            if (traceLevel > 1) traceInfo += "n  R" + r + " (Key = " + ByteHexUtil.toHEX1(Kdr) + ")t";
            ...
            if (traceLevel > 2) traceInfo += "ntISR = " + ByteHexUtil.toHEX1(ta);

            // InvSubBytes(state) into a using inverse S-box Si
            for (i = 0; i < BLOCK_SIZE; i++) a[i] = Si[ta[i] & 0xFF];
            if (traceLevel > 2) traceInfo += "ntISB = " + ByteHexUtil.toHEX1(a);

            // AddRoundKey(state) into ta
            for (i = 0; i < BLOCK_SIZE; i++) ta[i] = (byte) (a[i] ^ Kdr[i]);
            if (traceLevel > 2) traceInfo += "nt AK = " + ByteHexUtil.toHEX1(ta);
        ...
        if (traceLevel > 1) traceInfo += "n  R" + numRounds + " (Key = " + ByteHexUtil.toHEX1(Kdr) + ")t";
        ...
        if (traceLevel > 2) traceInfo += "ntISR = " + ByteHexUtil.toHEX1(a);
    ...
    public void setKey(byte[] key) {
        ...
        traceInfo = "";            // reset trace info
        if (traceLevel > 0) traceInfo = "setKey(" + ByteHexUtil.toHEX1(key) + ")n";
        ...
        if (traceLevel > 3) {
            ...
            for (r = 0; r < numRounds + 1; r++) traceInfo += "  R" + r + "t = " + ByteHexUtil.toHEX1(Ke[r]) + "n";
            traceInfo += "  Decrypt Round keys:n";
            for (r = 0; r < numRounds + 1; r++) traceInfo += "  R" + r + "t = " + ByteHexUtil.toHEX1(Kd[r]) + "n";
        }
    }
    ...
    public static void self_test(String hkey, String hplain, String hcipher, int lev) {
        ...
        byte[] key = ByteHexUtil.hex2byte(hkey);
        byte[] plain = ByteHexUtil.hex2byte(hplain);
        byte[] cipher = ByteHexUtil.hex2byte(hcipher);
        byte[] result;
        ...
        if (Arrays.equals(result, cipher))
            ...
            System.out.print("Test Failed. Result was " + ByteHexUtil.toHEX(result) + "n");
  ...
  public  static byte[] append(byte[] data) {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("data = " + ByteHexUtil.toHEX1(t1));
