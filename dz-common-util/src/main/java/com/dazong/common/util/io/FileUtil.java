...
import com.dazong.cc.common.util.Assert;
...
public class Files {
    ...
    public static String read(String path) {
        ...
        if (null == f)
            throw Throwables.makeThrow("Can not find file '%s'", path);
        return read(f);
    }
    ...
    public static void write(File f, Object obj) {
        if (null == f || null == obj)
            return;
        if (f.isDirectory())
            throw Throwables.makeThrow("Directory '%s' can not be write as File", f);
    ...
    public static File createFileIfNoExists(String path) throws IOException {
        ...
        if (!f.exists())
            Files.createNewFile(f);
        if (!f.isFile())
            throw Throwables.makeThrow("'%s' should be a file!", path);
        return f;
    }
    ...
    public static File createFileIfNoExists(File f) {
        ...
        if (!f.isFile())
            throw Throwables.makeThrow("'%s' should be a file!", f);
        return f;
    }
    ...
    public static File createDirIfNoExists(String path) {
        ...
        if (!f.exists())
            Files.makeDir(f);
        if (!f.isDirectory())
            throw Throwables.makeThrow("'%s' should be a directory!", path);
        return f;
    }
    ...
    public static File createDirIfNoExists(File d) {
        ...
        if (!d.exists())
            Files.makeDir(d);
        if (!d.isDirectory())
            throw Throwables.makeThrow("'%s' should be a directory!", d);
        return d;
    }
    ...
    public static File findFile(String path, ClassLoader klassLoader) {
        return findFile(path, klassLoader, Charsets.defaultEncoding());
    }
    ...
    public static File findFile(String path) {
        return findFile(path, ClassWrapper.getClassLoader(), Charsets.defaultEncoding());
    }
    ...
    public static InputStream findFileAsStream(String path, Class<?> klass) {
        return findFileAsStream(path, klass, Charsets.defaultEncoding());
    }
    ...
    public static InputStream findFileAsStream(String path) {
        return findFileAsStream(path, Files.class, Charsets.defaultEncoding());
    }
    ...
    public static Properties loadPropertiesFile(String loadPath) {
    	Assert.notBlank(loadPath, "the properites file path can't be null, please check it!");
        Properties prop = new Properties();
        try {
        	loadPath = Strings.trim(loadPath);
        	if (loadPath.indexOf("/") == 0) {
        		prop.load(Files.class.getResourceAsStream(loadPath));
        	} else {
        		prop.load(Thread.currentThread().getContextClassLoader().getResourceAsStream(loadPath));
        	}
        } catch (IOException e) {
            Throwables.wrapThrow(e, "load properties file['%s'] is error,cause is:'%s'", loadPath, e.getMessage());
        }
        return prop;
    }
}
