...
public class ClassWrapper<T> {
    ...
    public Object invoke(final Object object, final String methodName,
        ...
        if (method == null) {
            throw new PlatformException(CommonStatus.FAIL, "Could not find method [%s] on target [%s]", klass.getName(), methodName, object);
        }
        method.setAccessible(true);
        try {
            return method.invoke(object, parameters);
        } catch (Exception e) {
            throw new PlatformException(e, CommonStatus.FAIL, "An error on invoke method[%s] on target [%s]", method, object);
        }
    }
    ...
    public <AT extends Annotation> Field getField(Class<AT> ann) throws NoSuchFieldException {
        ...
        throw new PlatformException(CommonStatus.FAIL, "Can NOT find field [@%s] in class [%s] and it's parents classes",
                ann.getName(),
                klass.getName());
    }
    ...
    private static RuntimeException makeSetValueException(Class<?> type,
                                                          ...
                                                          Object value,
                                                          Exception e) {
        throw new PlatformException(e, CommonStatus.FAIL, "Fail to set value [%s] to [%s]->[%s] because '%s'", value, type.getName(), name, e.getMessage());
    }
    ...
    private static RuntimeException makeGetValueException(Class<?> type, String name) {
        return new PlatformException(CommonStatus.FAIL, "Fail to get value for [%s]->[%s]",
                type.getName(),
                name);
    }
...
//        }
    ...
    public Class<?> getPrimitiveWrapClass() {
        if (!klass.isPrimitive()) {
            ...
            throw new PlatformException(CommonStatus.FAIL, "Class '%s' should be a primitive class", klass.getName());
        ...
        throw new PlatformException(CommonStatus.FAIL, "Class [%s] has no wrapper class!", klass.getName());
    }
